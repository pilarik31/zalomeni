...
class Zalomeni {
	...
	const DEFAULT_CONJUCTIONS = '';

	const DEFAULT_CONJUCTIONS_LIST = 'a, i, o, u';
	...
	public static function add_options() {
		...
		add_option( 'zalomeni_conjunctions', self::DEFAULT_CONJUCTIONS );
		add_option( 'zalomeni_conjunctions_list', self::DEFAULT_CONJUCTIONS_LIST );
	...
	function admin_init() {
		...
		add_settings_field(
			...
			__( 'Předložky', 'zalomeni' ),
			'Zalomeni::settings_field_checkbox',
			'reading',
			'zalomeni_section',
			array(
				'option'                => 'prepositions',
				'description'           => 'Vkládat pevnou mezeru za následující předložky.',
				'toggle_list_read_only' => true,
			)
		);
		...
		add_settings_field(
			...
			'',
			'Zalomeni::settings_field_textlist',
			'reading',
			'zalomeni_section',
			array(
				'option'      => 'prepositions',
				'description' => '(oddělte jednotlivé předložky čárkou)',
			)
		);
		...
		add_settings_field(
			...
			array(
				'option'                => 'conjunctions',
				'description'           => 'Vkládat pevnou mezeru za následující spojky.',
				'toggle_list_read_only' => true,
			)
		);
		...
		add_settings_field(
			...
			array(
				'option'      => 'conjunctions',
				'description' => '(oddělte jednotlivé spojky čárkou)',
			)
		);
		...
		add_settings_field(
			...
			array(
				...
				'description'           => 'Vkládat pevnou mezeru za následující zkratky.',
				'toggle_list_read_only' => true,
			)
		);
		...
		add_settings_field(
			...
			array(
				'option'      => 'abbreviations',
				'description' => '(oddělte jednotlivé zkratky čárkou)',
			)
		);

		add_settings_field(
			'zalomeni_between_number_and_unit',
			__( 'Jednotky a míry', 'zalomeni' ),
			'Zalomeni::settings_field_checkbox',
			'reading',
			'zalomeni_section',
			array(
				'option'                => 'between_number_and_unit',
				'description'           => 'Vkládat pevnou mezeru mezi číslovku a jednotku míry (měrné jednotky, měna apod., např. <em>5 m</em> nebo <em>10 kg</em>).',
				'toggle_list_read_only' => true,
			)
		);
		...
		add_settings_field(
			...
			array(
				'option'      => 'between_number_and_unit',
				'description' => '(oddělte jednotlivé míry čárkou)',
			)
		);

		add_settings_field(
			'zalomeni_space_between_numbers',
			__( 'Mezery uprostřed čísel', 'zalomeni' ),
			'Zalomeni::settings_field_checkbox',
			'reading',
			'zalomeni_section',
			array(
				'option'      => 'space_between_numbers',
				'description' => 'Pokud jsou dvě čísla oddělena mezerou, předpokládat, že se jedná o formátování čísla pomocí mezery (např. telefonní číslo <em>800 123 456</em>) a nahrazovat mezeru pevnou mezerou, aby nedošlo k zalomení řádku uprostřed čísla.',
			)
		);

		add_settings_field(
			'zalomeni_space_after_ordered_number',
			__( 'Řadové číslovky', 'zalomeni' ),
			'Zalomeni::settings_field_checkbox',
			'reading',
			'zalomeni_section',
			array(
				'option'      => 'space_after_ordered_number',
		...
		add_settings_field(
			...
			__( 'Měřítka a poměry', 'zalomeni' ),
			'Zalomeni::settings_field_checkbox',
			'reading',
			'zalomeni_section',
			array(
				'option'      => 'spaces_in_scales',
				'description' => 'Pevné mezery v měřítkách a poměrech (např. <em>1 : 50 000</em>)',
			)
		);
	...
	public static function settings_field_custom_terms() {
		...
		. get_option( 'zalomeni_custom_terms', self::DEFAULT_CUSTOM_TERMS )
	...
	private function add_update_option_hooks() {
		...
		foreach ( array(
			'update_option_zalomeni_prepositions',
			...
			'update_option_zalomeni_custom_terms',
		) as $i ) {
	...
	public static function texturize( $text ) {
		...
		if ( get_option( 'zalomeni_matches' ) == '' ) {
			return $text; // no settings? then fall-back to just return the content
		}
		...
		for ( $i = 0; $i < $stop; $i++ ) {
			...
			if ( ! empty( $curl ) ) {
				...
				} elseif ( version_compare( $wp_version, '2.9', '<' ) ) {
